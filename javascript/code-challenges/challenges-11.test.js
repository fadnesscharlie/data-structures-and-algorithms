...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here...
  return input.map(index => index.filter(num => typeof num === 'number'? num % 5 ===0: '').map(ex => 2 ** ex));
};
...
/* ------------------------------------------------------------------------------------------------
  ...
  let findMaleAndFemale = (data) => {
    return data.filter((char) => char.gender === 'male' || char.gender === 'female').map((person) => person.name).join(' and ');
  };
