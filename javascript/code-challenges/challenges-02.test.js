...
const raisedToTheThird = (arr) => {
  ...
  let poweredExponent = [];
  arr.forEach(power => {
    poweredExponent.push(Math.pow(power, 3));
  });
  return poweredExponent;
};
...
const addOne = (arr) => {
  // Solution code here...
  let addedOne = arr.map(added => added + 1);
  return addedOne;
};
...
const addQuestion = (arr) => {
  // Solution code here...
  let questionMarked = arr.map(string => string + '?');
...
const forLoopTwoToThe = (arr) => {
  ...
  for (let integer of arr) {
    poweredArray.push(Math.pow(2, integer));
  }
  return poweredArray;
};
...
const forEachTwoToThe = (arr) => {
  ...
  let poweredArray = [];
  arr.forEach(exp => {
    poweredArray.push(Math.pow(2, exp));
  });
  return poweredArray;
};
...
const mapTwoToThe = (arr) => {
  // Solution code here...
  let poweredArray = arr.map(exp => Math.pow(2, exp));
  return poweredArray;
};
...
const charCode = (arr) => {
  // Solution code here...
  let mappedIndex = arr.map(letter => letter.charCodeAt());
  return mappedIndex;
};
...
const extractAbilities = (arr) => {
  // Solution code here...
  let abilityName = [];
  arr.map((abilName, idx) => {
    abilityName.push(snorlaxAbilities.abilities[idx].ability.name);
  });
  return abilityName;
};
...
const extractStats = (arr) => {
  // Solution code here...
  let nameAndStats = [];
  let objNameAndStats = new Object();
  // let objNameAndStatsName;
  // let objNameAndStatsTotal;
  arr.map((snorName, idx) => {

    let effort = +snorlaxStats.stats[idx].effort;
    let baseStat = +snorlaxStats.stats[idx].baseStat;
    // objNameAndStatsName = (snorlaxStats.stats[idx].stat.name);
    // objNameAndStatsTotal = effort + baseStat;

    objNameAndStats.name = (snorlaxStats.stats[idx].stat.name);
    objNameAndStats.total = effort + baseStat;
    function hi() {
      nameAndStats.map(item => {
        nameAndStats[item].name, nameAndStats[item].total;
        return nameAndStats;
      });
    }
    hi();
  });
  nameAndStats.push(objNameAndStats);

  var result = {};
  for (var i = 0; i < nameAndStats.length; i++) {
    result[nameAndStats[i].name] = nameAndStats[i].total;
  }


  // console.log(nameAndStats);

  // let final = {
  //   name: objNameAndStatsName,
  //   total: objNameAndStatsTotal,
  // };
  // nameAndStats.push(final);

  // const extractStats = objNameAndStats=> objNameAndStats.map(item => objNameAndStats[item].name, objNameAndStats[item].total);

  return nameAndStats;
  // return result;
};

const secondTry = (arr) => arr.map(item => ({
  name: item.stat.name, total: item.effort + item.baseStat
}));
